...
usage() {
  echo "Usage: $0 [--audio-samples N] [--max-audios N] [--queries-per-audio N] [--audio-clip-seconds SECONDS]" >&2
  echo "  Builds/resumes the CLAP caption-to-audio benchmark in data/clap_benchmark." >&2
  echo "  --max-audios is total audio candidates per caption query, including the positive." >&2
}
