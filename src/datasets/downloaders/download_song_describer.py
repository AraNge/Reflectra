...
def download_song_describer_samples(number: int):
    ...
    target_count = min(number, len(ds))
    existing_records: Dict[str, Dict[str, Any]] = {}
    if os.path.exists(metadata_output_path):
        with open(metadata_output_path, "r", encoding="utf-8") as meta_file:
            for line in meta_file:
                ...
                if audio_id and audio_path and os.path.exists(audio_path):
                    existing_records[audio_id] = record

    saved_records: Dict[str, Dict[str, Any]] = dict(existing_records)
    failed_count = 0

    if len(saved_records) >= target_count:
        print(
            f"Existing usable Song Describer metadata already has "
            f"{len(saved_records)}/{target_count} samples."
        )
    else:
        for idx, row in enumerate(tqdm(ds, total=len(ds))):
            ...
            if audio is None:
                failed_count += 1
                print(f"\n[{idx + 1}/{len(ds)}] Failed: no audio field found.")
                continue
            ...
            if os.path.exists(file_path):
                print(f"\n[{idx + 1}/{len(ds)}] Already exists: {file_path}")
            else:
                print(
                    f"\n[{idx + 1}/{len(ds)}] "
                    f"Saving Song Describer audio: {audio_id}"
                )
            ...
            saved_records[audio_id] = metadata

    with open(metadata_output_path, "w", encoding="utf-8") as meta_file:
        for metadata in list(saved_records.values())[:target_count]:
            meta_file.write(json.dumps(metadata, ensure_ascii=False) + "\n")

    print("\nDownload summary:")
    print(f"Split: {split}")
    print(f"Requested: {target_count}")
    print(f"Saved audio files: {min(len(saved_records), target_count)}")
